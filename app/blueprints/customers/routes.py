...
CUSTOMER_RESOURCES: dict[str, ResourceConfig] = {
    "customers": ResourceConfig(
        ...
        columns=[
            ("Name", lambda item: item.full_name),
            ("Email", lambda item: item.email),
            ("Phone", lambda item: item.phone),
            ("Active", lambda item: item.is_active),
        ],
    ),
}
...
def customers_root():
    return redirect(url_for("customers.list_resource", resource_key="customers"))


@bp.route("/<resource_key>/")
@roles_required(UserRole.ADMIN, UserRole.STAFF)
def list_resource(resource_key: str):
    if resource_key not in CUSTOMER_RESOURCES:
        return render_template("errors/404.html"), 404
    config = CUSTOMER_RESOURCES[resource_key]
...
@roles_required(UserRole.ADMIN, UserRole.STAFF)
def create_resource(resource_key: str = "customers"):
    if resource_key not in CUSTOMER_RESOURCES:
        return render_template("errors/404.html"), 404
    config = CUSTOMER_RESOURCES[resource_key]
    form = config.form_class()
    if form.validate_on_submit():
        ...
        return redirect(
            url_for("customers.detail_resource", resource_key=resource_key, resource_id=instance.id)
        )
    return render_template(
        "dashboard/resource_form.html", resource=config, form=form, mode="create"
    )
...
@roles_required(UserRole.ADMIN, UserRole.STAFF)
def detail_resource(resource_id: int, resource_key: str = "customers"):
    if resource_key not in CUSTOMER_RESOURCES:
        return render_template("errors/404.html"), 404
    config = CUSTOMER_RESOURCES[resource_key]
...
@roles_required(UserRole.ADMIN, UserRole.STAFF)
def edit_resource(resource_id: int, resource_key: str = "customers"):
    if resource_key not in CUSTOMER_RESOURCES:
        return render_template("errors/404.html"), 404
    config = CUSTOMER_RESOURCES[resource_key]
    instance = get_by_id(config.model, resource_id)
...
@roles_required(UserRole.ADMIN, UserRole.STAFF)
def archive_resource_view(resource_id: int, resource_key: str = "customers"):
    if resource_key not in CUSTOMER_RESOURCES:
        return render_template("errors/404.html"), 404
    config = CUSTOMER_RESOURCES[resource_key]
    instance = get_by_id(config.model, resource_id)
