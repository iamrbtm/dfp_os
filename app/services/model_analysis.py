...
def _parse_gcode_stats(gcode_path: str | Path) -> dict | None:
    ...
    grams_pattern = re.compile(
        r";\s*total filament used\s*\[g\]\s*=\s*([\d.]+)", re.IGNORECASE
    )
    volume_pattern = re.compile(
        r";\s*filament used\s*\[cm3\]\s*=\s*([\d.]+)", re.IGNORECASE
    )
    time_pattern = re.compile(
        r";\s*estimated printing time\s*\(normal mode\)\s*=\s*(.+)",
        re.IGNORECASE,
    )
    ...
    for line in tail:
        if not found_filament:
            ...
            if m and not found_filament:
                val = Decimal(m.group(1))
                if val > 0:
                    filament_grams = (val * PLA_DENSITY_G_PER_CM3).quantize(
                        Decimal("0.01")
                    )
                    found_filament = True
