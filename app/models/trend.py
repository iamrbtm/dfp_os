...
class TrendSnapshot(PrimaryKeyMixin, TimestampMixin, db.Model):
    ...
    keyword_or_category: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
...
class TrendReport(PrimaryKeyMixin, TimestampMixin, db.Model):
    ...
    top_opportunities: Mapped[list | None] = mapped_column(JSON, nullable=True)
    growing_categories: Mapped[list | None] = mapped_column(JSON, nullable=True)
    declining_trends: Mapped[list | None] = mapped_column(JSON, nullable=True)
    pipeline_meta: Mapped[dict | None] = mapped_column(JSON, nullable=True)
