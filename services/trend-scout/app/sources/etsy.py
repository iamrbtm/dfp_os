    ...
    for query in SEED_QUERIES:
        ...
        try:
            ...
            if resp.status_code == 200:
                    ...
                    result.items.append(
                        {
                            ...
                            "thumbnail": (
                                item.get("images", [{}])[0].get("url_570xN", "") if item.get("images") else ""
                            ),
                            "price": price,
                            "currency": currency,
                            "tags": item.get("tags", []),
                            ...
                            "shop_name": (
                                item.get("Shop", {}).get("shop_name", "") if isinstance(item.get("Shop"), dict) else ""
                            ),
                        }
                    )
                ...
                result.errors.append("HTTP 401 - Invalid or missing Etsy API key. Verify ETSY_API_KEY is correct.")
            else:
