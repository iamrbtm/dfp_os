...
from app.sources._base import ScoutResult, request_with_retry
    ...
    if not pinterest_api_key:
        results.append(
            ScoutResult(
                ...
                errors=[
                    "PINTEREST_API_KEY environment variable not set. Get a key at https://developers.pinterest.com/"
                ],
            )
        )
        return results
    ...
    for query in SEED_QUERIES:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                result.errors.append("HTTP 401 - Invalid Pinterest API key. Verify PINTEREST_API_KEY is correct.")
            elif resp.status_code == 403:
                result.errors.append(
                    "HTTP 403 - Pinterest API access denied. Your app may not have the required scopes."
                )
            else:
