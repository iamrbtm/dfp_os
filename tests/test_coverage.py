...
def test_create_print_job_from_order_item_custom_label(app):
    with app.app_context():
        ...
        db.session.add(item)
        db.session.commit()
        job = print_job_svc.create_print_job_from_order_item(
            item, label="Custom Label", priority=3, estimated_minutes=60
        )
...
def test_staff_user_can_access_staff_routes(client, app):
    with app.app_context():
        staff = User(
            email="staff-auth-test@example.com",
            first_name="Staff",
            last_name="T",
            role=UserRole.STAFF,
            is_active=True,
...
def _api_token_header(client, scopes=None):
    with client.application.app_context():
        user = User(
            email="api-post-user@example.com",
            first_name="API",
            last_name="Post",
            role=UserRole.ADMIN,
            is_active=True,
...
def test_api_post_category(client):
    ...
    r = client.post(
        "/api/v1/categories",
        json={"name": "API Cat", "slug": "api-cat"},
...
def test_api_post_collection(client):
    ...
    r = client.post(
        "/api/v1/collections",
        json={"name": "API Coll", "slug": "api-coll"},
        headers={"Authorization": f"Bearer {token}"},
    )
    assert r.status_code == 201
...
def test_api_get_single_not_found(client):
    token = _api_token_header(client)
    r = client.get("/api/v1/products/99999", headers={"Authorization": f"Bearer {token}"})
...
def test_api_delete_product(client):
    ...
    with client.application.app_context():
        ...
        db.session.commit()
        pid = p.id
    r = client.delete(f"/api/v1/products/{pid}", headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 200
...
def test_api_delete_not_found(client):
    token = _api_token_header(client)
    r = client.delete("/api/v1/products/99999", headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 404
...
def test_admin_archive_customer(client, login_admin):
    with client.application.app_context():
        c = Customer(
            first_name="Archive", last_name="Me", email="archive-me@example.com", is_active=True
        )
        db.session.add(c)
        db.session.commit()
        cid = c.id
...
def test_order_item_form_apply_custom(app):
    with app.app_context():
        ...
        item = OrderItem(
            order=o,
            is_custom_item=True,
            custom_description="Custom widget",
            quantity=1,
...
def test_print_job_default_status(app):
    with app.app_context():
        job = PrintJob(
            status=PrintJobStatus.QUEUED, priority=0, estimated_minutes=0, label="Default"
        )
        db.session.add(job)
        db.session.commit()
        assert job.status == PrintJobStatus.QUEUED
